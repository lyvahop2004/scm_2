...
import sendOrderConfirmationEmail from "../controllers/orderController";
...
let initwebRoutes = (app) => {
  ...
  //=================API ORDER=============================//
  router.post(
    "/api/sendOrderConfirmationEmail",
  ...
  //   router.post("/sendOrderEmail", orderController.sendOrderEmailToUser);
...
router.post("/sendOrderEmail", async (req, res) => {
  const { userEmail, orderDetails } = req.body; // Nhận dữ liệu từ frontend (email và chi tiết đơn hàng)

  // Kiểm tra nếu thiếu thông tin
  if (!userEmail || !orderDetails) {
    return res.status(400).send("Thiếu thông tin email hoặc chi tiết đơn hàng");
  }

  try {
    // Gửi email
    await sendOrderEmailToUser(userEmail, orderDetails);
    return res.status(200).send("Email đã được gửi thành công");
  } catch (error) {
    console.log(error);
    return res.status(500).send("Lỗi khi gửi email: " + error.message);
  }
});

// router.get("/api/get-detail-address-user", async (req, res) => {
...
//   try {
//     const address = await addressUserController.getAddressById(id); // Gọi hàm controller để lấy dữ liệu
//     if (address) {
...
//       return res
//         .status(404)
//         .json({ errCode: 2, message: "Không tìm thấy địa chỉ" });
//     }
//   } catch (error) {
//     console.error("Lỗi khi lấy địa chỉ người dùng:", error);
//     return res.status(500).json({ errCode: 3, message: "Lỗi server" });
//   }
// });
