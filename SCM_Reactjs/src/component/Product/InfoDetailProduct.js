...
function InfoDetailProduct(props) {
  ...
  let handleAddShopCart = () => {
    if (props.userId) {
      ...
      // Chuyển trang sang giỏ hàng sau khi hiển thị thông báo
      setTimeout(() => {
        window.location.href = "/shopcart"; // Điều hướng trực tiếp đến trang cart
  ...
  const handleQuantityChange = (event) => {
    const value = parseInt(event.target.value, 10); // Chuyển thành số nguyên

    if (isNaN(value) || value < 1) {
      // Khi giá trị không hợp lệ hoặc nhỏ hơn 1
      toast.error("Số lượng không được nhỏ hơn 1");
      setquantityProduct(1);
    } else if (value > quantity) {
      // Khi giá trị vượt quá số lượng trong kho
      toast.error(`Chỉ còn ${quantity} sản phẩm trong kho`);
      setquantityProduct(quantity);
    } else {
      setquantityProduct(value); // Cập nhật số lượng hợp lệ
    }
  };
  return (
